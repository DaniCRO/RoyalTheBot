...
class Fun():
	...
	async def lball(self, ctx, question = None):
		...
		-------------------
		Ex:
		r!8ball Is this the best bot?'''
		if question is None:
			return await ctx.send('Hey, please do `r!8ball <question>`')
	...
	async def space(self, ctx, *, message=None):
		...
		a?space Hello There'''
		if message is None:
			return await ctx.send('\Hey, please do `r!space [message]`!')
